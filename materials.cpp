...
Materials::Materials(sdl::Window &aWindow, int aWidth, int aHeight)
  : window(aWindow), width(aWidth), height(aHeight)
{
  ...
  materialDiffuse = bgfx::createUniform("materialDiffuse", bgfx::UniformType::Vec4);
  materialSpecular = bgfx::createUniform("materialSpecular", bgfx::UniformType::Vec4);
  ...
  lightSpecular = bgfx::createUniform("lightSpecural", bgfx::UniformType::Vec4);

  program = loadProgram("materials-vs", "materials-fs");
  programLight = loadProgram("materials-vs", "materials-light-fs");
}
...
auto Materials::update() -> void
{
...

  {
    ...
    glm::vec3 lightColor;
    lightColor.x = sin(secs * 2.0f);
    lightColor.y = sin(secs * 0.7f);
    lightColor.z = sin(secs * 1.3f);
    ...
    bgfx::setUniform(materialDiffuse, std::array{1.f, .5f, .31f, 0.0f}.data());
    bgfx::setUniform(materialSpecular, std::array{.5f, .5f, .5f, 0.0f}.data());
    bgfx::setUniform(materialShininess, std::array{32.f, 0.0f, 0.0f, 0.0f}.data());
...
auto Materials::strafe(float forward, float right) -> void
{
  camPos += 0.2f *
...
auto Materials::look(float dx, float dy) -> void
{
  camYaw -= dx;
  camPitch += dy;
  camPitch =
