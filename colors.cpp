...
Colors::Colors(sdl::Window &aWindow, int aWidth, int aHeight)
  : window(aWindow), width(aWidth), height(aHeight)
{
  ...
  program = loadProgram("colors-vs", "colors-fs");
  programLight = loadProgram("colors-vs", "colors-light-fs");
}
...
auto Colors::strafe(float forward, float right) -> void
{
  camPos += 0.2f *
...
auto Colors::look(float dx, float dy) -> void
{
  camYaw -= dx;
  camPitch += dy;
  camPitch =
