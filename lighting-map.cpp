...
LightingMap::LightingMap(sdl::Window &aWindow, int aWidth, int aHeight)
  : window(aWindow), width(aWidth), height(aHeight)
{
  ...
  program = loadProgram("lighting-map-vs", "lighting-map-fs");
  programLight = loadProgram("lighting-map-vs", "lighting-map-light-fs");
}
...
auto LightingMap::strafe(float forward, float right) -> void
{
  camPos += 0.2f *
...
auto LightingMap::look(float dx, float dy) -> void
{
  camYaw -= dx;
  camPitch += dy;
  camPitch =
